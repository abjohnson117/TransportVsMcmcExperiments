...
rej_samples = np.load(f"samp_results/_{seed}/rej_samps.npy")
nn_samples = (np.load(f"nn_results/ode_{seed}/nn_samps.npy"))
# nn_sde_samples = (np.load(f"nn_sde_results/ode_{seed}/nn_samps.npy").T)[:, 1::2]

wd_nn_array = np.zeros(len(conditioning_list))
# wd_nn_sde_array = np.zeros(len(conditioning_list))
wd_mcmc_array = np.zeros(len(conditioning_list))
wd_nn_array_std = np.zeros(len(conditioning_list))
# wd_nn_sde_array_std = np.zeros(len(conditioning_list))
...
for i, cond_num in enumerate(tqdm(conditioning_list)):
    ...
    for j in range(cond_num):
        u_nn_weights = np.ones(len(nn_samps[:, j].reshape(-1)))/len(nn_samps[:, j].reshape(-1))
        v_weights = np.ones(len(rej_samps[:, j].reshape(-1))) / len(rej_samps[:, j].reshape(-1))
        u_mcmc_weights = np.ones(len(mcmc_samps[:, j].reshape(-1))) / len(mcmc_samps[:, j].reshape(-1))
        nn_array[j] = wd(nn_samps[:, j].reshape(-1), rej_samps[:, j].reshape(-1), u_weights=u_nn_weights, v_weights=v_weights, p=2)
        # sde_array[j] = wd(sde_samps[:, j].reshape(-1), rej_samps[:, j].reshape(-1), p=2)
        mcmc_array[j] = wd(mcmc_samps[:, j].reshape(-1), rej_samps[:, j].reshape(-1), u_weights=u_mcmc_weights, v_weights=v_weights, p=2)
    ...
    # wd_nn_sde_array_std[i] = np.std(sde_array)
    wd_mcmc_array_std[i] = np.std(mcmc_array)

np.save(os.path.join(output_dir, "wd_nn_array_mean.npy"), wd_nn_array)
# np.save(os.path.join(output_dir, "wd_nn_sde_array_mean.npy"), wd_nn_sde_array)
np.save(os.path.join(output_dir, "wd_nn_array_std.npy"), wd_nn_array_std)
# np.save(os.path.join(output_dir, "wd_nn_sde_array_std.npy"), wd_nn_sde_array_std)
