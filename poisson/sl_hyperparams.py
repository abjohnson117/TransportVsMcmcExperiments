...
class SiOdeSmac:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        hidden_layer = Integer("hidden_layer", (512, 1100), default=512, log=True)
        num_hidden_layers = Integer("num_hidden_layers", (5, 10), default=8, log=True)
        batch_size = Integer("batch_size", (100, 1300), default=128, log=True)
    ...
    def train(self, config: Configuration, seed: int = 0) -> float:
        ...
        try:
            ...
            if isinstance(cond_samples, list):
                ...
                u_samples = u_samples.reshape(nsamples, nx, ny)
                u_samples += extra_samp
                print(f"These are the first 5 samples: {u_samples[0, :5]}")
                swd_rel_error = swd(
                    np.array(u_samples).reshape(nsamples, nx * ny),
                    np.array(hmala_samples),
                    n_projections=n_projections,
                    seed=SEED,
                ) / base_swd
                loss = swd_rel_error.item()
                print(f"This is the swd rel error: {loss}")
...
flat_length = nx * ny
hmala_root = "sl-data"
hmala_samples = np.load(os.path.join(hmala_root, "hmala_samples.npy"))
# chains = []
...
train_dim = 50000
ys = (np.load("sl-data/solutions_grid.npy"))[:train_dim]
us = (np.load("sl-data/parameters.npy"))[:train_dim, :].reshape(
    train_dim, flat_length
)

us_ref = (np.load("sl-data/parameters.npy"))[
    train_dim : train_dim * 2, :
].reshape(train_dim, flat_length)
np.random.shuffle(us_ref)

us_test = (np.load("sl-data/parameters.npy"))[
...
base_swd = swd(
    us_ref[random_idxs, :], hmala_samples, n_projections=n_projections, seed=SEED
)
# swd_med = swd(
...
cond_values = jnp.array(yu_true[: yu_dimension[0]])
...
for i, sample_no in enumerate(sample_no_list):
    run = wandb.init(
        ...
        project="Poisson - SI hyperparams - circle",
