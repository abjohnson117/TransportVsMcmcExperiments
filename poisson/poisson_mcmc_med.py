...
"""
hMALA sampling for the Poisson inverse problem — MEDIAN observation (data_50.npy).

Observation targets (sensor locations) are read from data.h5;
observation values are read from data_50.npy (the median-valued observation).
...
Usage (10 serial trials):
    for i in $(seq 0 9); do
        python poisson_mcmc_med.py --output_root mcmc_median_100k --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python poisson_mcmc_med.py --output_root mcmc_median_100k
"""
...
parser = argparse.ArgumentParser(description="hMALA chain — median observation")
parser.add_argument(
    "--output_root",
    type=str,
    required=False,
    default="mcmc_median_100k",
...
STEP_SIZE = 0.13      # hMALA step size (same as poisson.py)
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # Misfit — MEDIAN observation (data_50.npy)
    # -----------------------------------------------------------------------
    ...
    data    = np.load("data_50.npy")
    print(f"Loaded targets from data.h5 (ntargets={targets.shape[0]})")
    print(f"Loaded observation values from data_50.npy (shape={data.shape})")
    ...
    mcmc_stats = {
        ...
        "chain_rank":       RANK,
        "observation":      "median (data_50.npy)",
    }
