...
"""
hMALA sampling for the Poisson inverse problem — MAIN observation (data.h5).
...
Usage (10 serial trials):
    for i in $(seq 0 9); do
        python poisson_mcmc_main.py --output_root mcmc_main --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python poisson_mcmc_main.py --output_root mcmc_main
"""
...
parser = argparse.ArgumentParser(description="hMALA chain — main observation")
parser.add_argument(
    "--output_root",
    type=str,
    required=False,
...
if __name__ == "__main__":
    ...
    # Pickle the method-list summary (Samples field replaced by numpy array)
    method_summary = {
        "hMALA": {
            "Options":     opts,
