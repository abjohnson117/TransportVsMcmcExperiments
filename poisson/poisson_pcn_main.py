...
"""
pCN sampling for the Poisson inverse problem — MAIN observation (data.h5).
...
Usage (10 serial trials):
    for i in $(seq 0 9); do
        python poisson_pcn_main.py --output_root pcn_main --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python poisson_pcn_main.py --output_root pcn_main
"""
...
parser = argparse.ArgumentParser(description="pCN chain — main observation")
parser.add_argument(
    "--output_root",
    type=str,
    required=False,
...
STEP_SIZE = 0.02      # pCN step size (beta parameter, in (0, 1))
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # Misfit — MAIN observation (data.h5)
    # -----------------------------------------------------------------------
    ...
    print(f"Loaded observation from data.h5  (ntargets={targets.shape[0]})")
    ...
    # -----------------------------------------------------------------------
    # Initial point — MAP estimate
    # -----------------------------------------------------------------------
    x0 = hm.dlVector2npArray(x[hp.PARAMETER])
    print(f"[chain {RANK}] x0 initialised at MAP, x0[:5] = {x0[:5]}")
