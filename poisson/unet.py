...
class ResnetBlock(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mlp_layers_y = [
            jax.nn.silu,
            eqx.nn.Linear(y_emb_dim, dim_out, key=keys[1])
        ]
    ...
    def __call__(self, x, t, y, *, key):
        ...
        for layer in self.mlp_layers_t:
            t = layer(t)
...
class UNet(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.first_conv = eqx.nn.Conv2d(
            in_channels, hidden_size, kernel_size=3, padding=1, key=keys[2]
        )
    ...
    def __call__(self, t, x, y, v=None, *, key=None):
        ...
        if v is None:
            ...
            in_ = jnp.concatenate([x, v])
        h = self.first_conv(in_)
        hs = [h]
        for res_blocks in self.down_res_blocks:
            for res_block in res_blocks:
                key, subkey = key_split_allowing_none(key)
                h = res_block(h, t, y, key=subkey)
                hs.append(h)

        key, subkey = key_split_allowing_none(key)
        h = self.mid_block1(h, t, y, key=subkey)
        key, subkey = key_split_allowing_none(key)
        h = self.mid_block2(h, t, y, key=subkey)
        ...
        for res_blocks in self.ups_res_blocks:
            for res_block in res_blocks:
                ...
                if res_block.up:
                    ...
                    h = res_block(jnp.concatenate((h, hs.pop()), axis=0), t, y, key=subkey)
