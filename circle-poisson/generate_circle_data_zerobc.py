#!/usr/bin/env python3
"""
generate_circle_data_zerobc.py

Generates the out-of-distribution circle-phantom test point for the
zero-Dirichlet-everywhere Poisson inverse problem (the "_p" / p-poisson
variant used to build circle-poisson/training_dataset via
generate_training_dataset.py).

True parameter: smoothed circle phantom (same geometry as generate_circle_data.py)
  m(x) = log(3) + (log(8) - log(3)) * sigma(tau * (r0 - ||x - c||))

PDE setup matches generate_training_dataset.py (NOT generate_circle_data.py):
...
Outputs saved to circle-data-zerobc/:
  data_circle_zerobc.npy   concat [y_obs (100,) | u_true_flat (1089,)]
  targets_zerobc.npy       observation-point coordinates (100, 2)
  state_circle_zerobc.npy  full PDE solution on the 33x33 vertex grid
...
OUTPUT_DIR = os.path.join(HERE, "circle-data-zerobc")
...
def circle_phantom(n, radius=0.8, tau=40.0, value=None):
    """Smooth circle image of size n x n via a logistic (sigmoid) transition.

    m(x) = log(3) + (log(40) - log(3)) * sigma(tau * (r0 - ||x - c||))

    where c is the domain center, r0 = radius * 0.5 is the circle radius in
    normalized [0,1]^2 domain coordinates, and tau controls the sharpness of
    the transition (larger tau -> closer to the original piecewise-constant
    circle; smaller tau -> smoother/wider transition band).
    """
    if value is None:
        value = np.log(8)
    low, high = np.log(3), value
    xs = np.linspace(0.0, 1.0, n)
    ys = np.linspace(0.0, 1.0, n)
    X, Y = np.meshgrid(xs, ys)  # X, Y both shape (n, n), row-major (y, x)
    cx = cy = 0.5
    r0 = radius * 0.5
    dist = np.sqrt((X - cx) ** 2 + (Y - cy) ** 2)
    img = low + (high - low) * sigmoid(tau * (r0 - dist))
    return img
...
if __name__ == "__main__":
    ...
    # ── Circle-phantom true parameter ────────────────────────────────────────
    m_img_flat = circle_phantom(NX + 1).reshape((NX + 1) * (NY + 1))
    ...
    print(f"Circle phantom range: [{dof_vals.min():.3f}, {dof_vals.max():.3f}]"
          f"  (log3={np.log(3):.3f}, log8={np.log(8):.3f})")
    ...
    np.save(os.path.join(OUTPUT_DIR, "data_circle_zerobc.npy"), yu)
    np.save(os.path.join(OUTPUT_DIR, "state_circle_zerobc.npy"), state_flat)
    print(f"Saved  data_circle_zerobc.npy   shape={yu.shape}")
    print(f"Saved  state_circle_zerobc.npy  shape={state_flat.shape}  (full PDE solution on 33x33 grid)")
    ...
    for ax, arr, title in zip(
        ...
        ["Circle phantom (true parameter)", "MAP estimate (zero-BC)"],
    ):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "circle_true_and_map_zerobc.png"), dpi=100)
    plt.close()
