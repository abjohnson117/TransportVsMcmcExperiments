...
Generates the out-of-distribution circle-phantom test point for the Poisson
inverse problem.

True parameter: smoothed circle phantom
  m(x) = log(3) + (log(40) - log(3)) * sigma(tau * (r0 - ||x - c||))
  log(40) ≈ 3.69  near the center (inside radius r0 = 0.8 * half-domain-width)
  log(3)  ≈ 1.10  far from the center (outside r0)
  with a logistic transition of width ~1/tau around ||x - c|| = r0

This is strongly out of distribution relative to the BiLaplacian Gaussian prior
used to generate training data (which produces smooth random fields).
...
Outputs saved to circle-data/:
  data_circle.npy   concat [y_obs (100,) | u_true_flat (1089,)]
...
OUTPUT_DIR = os.path.join(HERE, "circle-data")
...
if __name__ == "__main__":
    ...
    np.save(os.path.join(OUTPUT_DIR, "data_circle.npy"), yu)
    np.save(os.path.join(OUTPUT_DIR, "state_circle.npy"), state_flat)
    print(f"Saved  data_circle.npy   shape={yu.shape}")
    print(f"Saved  state_circle.npy  shape={state_flat.shape}  (full PDE solution on 33x33 grid)")
    ...
    for ax, arr, title in zip(
        ...
        ["Circle phantom (true parameter)", "MAP estimate"],
    ):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "circle_true_and_map.png"), dpi=100)
    plt.close()
