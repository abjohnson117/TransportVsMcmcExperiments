...
"""
hMALA sampling for the Poisson inverse problem — circle phantom, ZERO-DIRICHLET-EVERYWHERE BC.

PDE/prior/noise setup matches generate_circle_data_zerobc.py and
...
This is the zero-BC counterpart of poisson_mcmc_circle.py, which instead uses
the u = x[1] top/bottom Dirichlet BC (the "_delta" training set).

Observation targets and values are read from circle-data-zerobc/
(written by generate_circle_data_zerobc.py).

Runs a single hMALA chain for 155,000 total MCMC steps.

Usage:
    python poisson_mcmc_circle_zerobc.py
"""
...
REL_NOISE  = 1e-8     # matches generate_circle_data_zerobc.py / generate_training_dataset.py
CIRCLE_DIR = Path('circle-data-zerobc')
...
if __name__ == "__main__":
    ...
    sep = "\n" + "#" * 80 + "\n"
    output_dir = "circle_mcmc_samps_zerobc"
    ...
    # -----------------------------------------------------------------------
    # Misfit — circle phantom observation (data_circle_zerobc.npy)
    # -----------------------------------------------------------------------
    targets = np.load(CIRCLE_DIR / 'targets_zerobc.npy')
    data_param = np.load(CIRCLE_DIR / 'data_circle_zerobc.npy')
    data = data_param[:100]
    param = data_param[100:]
    print(f"Loaded targets from {CIRCLE_DIR / 'targets_zerobc.npy'} (ntargets={targets.shape[0]})")
    print(f"Loaded observation values from data_circle_zerobc.npy (shape={data.shape})")
    print(f"noise_variance = {REL_NOISE:.4e}  (relative noise, matches generate_circle_data_zerobc.py)")
    ...
    ax[0].set_title("Circle Phantom")

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "mcmc_mean_circle_zerobc.png"))
    ...
    mcmc_stats = {
        ...
        "observation":      "Circle (data_circle_zerobc.npy, zero-Dirichlet-everywhere BC)",
    }
