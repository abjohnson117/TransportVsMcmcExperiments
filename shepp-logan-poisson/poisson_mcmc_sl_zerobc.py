...
"""
hMALA sampling for the Poisson inverse problem — Shepp-Logan phantom, ZERO-DIRICHLET-EVERYWHERE BC.

PDE/prior/noise setup matches generate_shepp_logan_data_zerobc.py and
generate_training_dataset.py (the "_p" training set): u = 0 on the entire
boundary, BiLaplacianPrior(gamma=1, delta=9), relative noise (REL_NOISE * ||d||_inf).

This is the zero-BC counterpart of poisson_mcmc_circle.py (which, despite its
name, is the MCMC script for the SL phantom in this folder), which instead
uses the u = x[1] top/bottom Dirichlet BC (the "_delta" training set).

Observation targets and values are read from shepp-logan-data-zerobc/
(written by generate_shepp_logan_data_zerobc.py).

Runs a single hMALA chain for 155,000 total MCMC steps.

Usage:
    python poisson_mcmc_sl_zerobc.py
"""
...
# ---------------------------------------------------------------------------
# MCMC parameters (hardcoded for 155 k-step runs)
# ---------------------------------------------------------------------------
N_SAMPLES  = 155000   # total MCMC steps (including burn-in)
BURNIN     = 5000     # steps discarded as burn-in
STEP_SIZE  = 0.1      # hMALA step size (same as poisson_mcmc_circle.py)
REL_NOISE  = 1e-8     # matches generate_shepp_logan_data_zerobc.py / generate_training_dataset.py
SL_DIR     = Path('shepp-logan-data-zerobc')
...
if __name__ == "__main__":
    ...
    sep = "\n" + "#" * 80 + "\n"
    output_dir = "sl_mcmc_samps_zerobc"
    ...
    # -----------------------------------------------------------------------
    # Misfit — Shepp-Logan phantom observation (data_sl_zerobc.npy)
    # -----------------------------------------------------------------------
    targets = np.load(SL_DIR / 'targets_zerobc.npy')
    data_param = np.load(SL_DIR / 'data_sl_zerobc.npy')
    data = data_param[:100]
    param = data_param[100:]
    print(f"Loaded targets from {SL_DIR / 'targets_zerobc.npy'} (ntargets={targets.shape[0]})")
    print(f"Loaded observation values from data_sl_zerobc.npy (shape={data.shape})")
    print(f"noise_variance = {REL_NOISE:.4e}  (relative noise, matches generate_shepp_logan_data_zerobc.py)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "mcmc_mean_sl_zerobc.png"))
    ...
    mcmc_stats = {
        ...
        "observation":      "SL (data_sl_zerobc.npy, zero-Dirichlet-everywhere BC)",
    }
