...
ker_samples = (np.load(f"kernel_results/ode_{seed}/k_samps.npy").T)[:, 1::2]

wd_ker_array = np.zeros(len(conditioning_list))
wd_ker_array_std = np.zeros(len(conditioning_list))
for i, cond_num in enumerate(tqdm(conditioning_list)):
    ...
    ker_samps = ker_samples[:, idxs]

    bad_cols = np.where(np.isnan(ker_samps).any(axis=0))[0]
    keep_cols = np.setdiff1d(np.arange(ker_samps.shape[1]), bad_cols)
    ker_samps = ker_samps[:, keep_cols]
    rej_samps = rej_samps[:, keep_cols]
    cond_vars = cond_vars[keep_cols]
    cond_num = cond_vars.shape[0]

    ker_array = np.zeros(cond_num)
    ...
    wd_ker_array_std[i] = np.std(ker_array)
    # wd_mcmc_array_std[i] = np.std(mcmc_array)

np.save(os.path.join(output_dir, "wd_ker_array_mean.npy"), wd_ker_array)
np.save(os.path.join(output_dir, "wd_ker_array_std.npy"), wd_ker_array_std)
