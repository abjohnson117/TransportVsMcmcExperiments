# -*- coding: utf-8 -*-
"""
Budget analysis for p-Poisson h-MALA chains.

For each budget point (chain_length, num_cond_vars), randomly selects
num_cond_vars observations from a reference chain directory and runs
...
parser = argparse.ArgumentParser(description="p-Poisson h-MALA budget analysis")
parser.add_argument("--run_id", type=int, default=0, help="Run ID for seeding and output naming")
parser.add_argument("--output_root", type=str, default="mcmc_256",
...
parser.add_argument("--chain_start", type=int, default=0,
                    help="First chain index to draw conditioning observations from (inclusive)")
parser.add_argument("--chain_end", type=int, default=865,
                    help="Last chain index to draw conditioning observations from (exclusive)")
...
# ---------------------------------------------------------------------------
# Problem geometry (must match ppoisson_box.yaml)
# ---------------------------------------------------------------------------
Length = 1.0
Width  = 1.0
Height = 0.05
...
if __name__ == "__main__":
    ...
    with open(os.path.join(_here, "ppoisson_box.yaml")) as fid:
    ...
    for i, chain_length in enumerate(tqdm(chain_list)):
        ...
        for j, (obs_idx, y_obs_data) in enumerate(zip(random_idxs, selected_y_obs)):
            ...
            print(f"  Building Laplace posterior (randomized SVD, k={inargs['MCMC'].get('lr_rank', 20)})...")
