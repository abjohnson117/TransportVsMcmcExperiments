# -*- coding: utf-8 -*-
"""
MALA sampling for the p-Poisson inverse problem — MAIN observation (data_obs.npy).

Observation values are read from data_obs.npy; target locations are the fixed
10x10 grid on the top surface (z=Height) matching generate_training_dataset.py.

Runs a single MALA chain for 50,000 total MCMC steps.
Trial index is controlled by --start_idx (plus MPI rank for parallel runs).

The initial point is drawn from the Laplace posterior (which improves
convergence), but the MALA proposal uses an isotropic (unpreconditonied)
Gaussian — no Hessian metric.

Usage (10 serial trials):
    for i in $(seq 0 9); do
        python ppoisson_mala_main.py --output_root mcmc_main_mala --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python ppoisson_mala_main.py --output_root mcmc_main_mala
"""
...
parser = argparse.ArgumentParser(description="MALA chain — p-Poisson main observation")
parser.add_argument("--output_root", type=str, default="mcmc_main_mala",
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # Misfit — MAIN observation (data_obs.npy)
    # -----------------------------------------------------------------------
    ...
    print(f"Loaded data_obs.npy (shape={data.shape})")
    ...
    # -----------------------------------------------------------------------
    # Laplace posterior approximation (for warm-start initialization only)
    # -----------------------------------------------------------------------
    print(sep, "Building Laplace posterior for warm-start initialization", sep)
    ...
    # Standard MALA: isotropic (identity) metric — no Hessian preconditioning.
    opts = dict(options)
    opts["StepSize"] = STEP_SIZE
    param_dim    = Vh[hp.PARAMETER].dim()
    gauss_isotropic = mm.Gaussian(np.zeros(param_dim))
    prop         = ms.MALAProposal(opts, problem, gauss_isotropic)
    kern         = ms.MHKernel(opts, problem, prop)
    sampler      = ms.SingleChainMCMC(opts, [kern])

    # -----------------------------------------------------------------------
    # Initial point — sample from Laplace posterior (deterministic per chain)
    # -----------------------------------------------------------------------
    ...
    # Run MCMC
    # -----------------------------------------------------------------------
    print(f"Starting MALA chain (N={N_SAMPLES}, BurnIn={BURNIN}) ...")
    ...
    np.save(os.path.join(output_dir, "mala_samples.npy"), vertex_samples)
    print(f"Saved {num_stored} samples to {output_dir}/mala_samples_delta.npy")
    ...
    with open(os.path.join(output_dir, "method-list-mala.pkl"), "wb") as ff:
