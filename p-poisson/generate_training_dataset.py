...
def generate_sample(mesh, Vh, pde, prior):
    ...
    try:
        ...
        # print(f"This is the shape of targets: {targets.shape}")
        # grid_1d = np.linspace(0.05, 0.95, 10)
        # X, Y = np.meshgrid(grid_1d, grid_1d)
        # print(f"This is X shape: {X.ravel()[None, :].shape}")
        # print(f"This is Y shape: {Y.ravel()[None, :].shape}")
        # print(f"This is Z shape: {dummy3.shape}")
        ...
        return parameter_array, solution_array, misfit.d.get_local(), targets
...
def main():
    ...
    targets = np.zeros((num_samples, ntargets, 3))
    ...
    # Save the datasets
    print("\nSaving datasets...")
    np.save(os.path.join(output_dir, "parameters_random_grid.npy"), parameters)
    np.save(os.path.join(output_dir, "solutions_full_random_grid.npy"), solutions)
    np.save(os.path.join(output_dir, "solutions_random_grid.npy"), misfits)
    np.save(os.path.join(output_dir, "locations_random_grid.npy"), targets)

    print("\nDataset generation complete!")
    print(f"Files are saved in the '{output_dir}' directory as:")
    print("- parameters_random_grid.npy")
    print("- solutions_full_random_grid.npy")
    print("- solutions_random_grid.npy")
    print("- locations_random_grid.npy")
