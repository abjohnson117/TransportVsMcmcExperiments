# -*- coding: utf-8 -*-
"""
hMALA sampling for the p-Poisson inverse problem — MEDIAN observation (data_50.npy).

Observation values are read from data_50.npy; target locations are the fixed
...
Usage (10 serial trials):
    for i in $(seq 0 9); do
        python ppoisson_mcmc_med.py --output_root mcmc_median --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python ppoisson_mcmc_med.py --output_root mcmc_median
"""
...
parser = argparse.ArgumentParser(description="hMALA chain — p-Poisson median observation")
parser.add_argument("--output_root", type=str, default="mcmc_median",
...
if __name__ == "__main__":
    ...
    print(f"Loaded data_50.npy (shape={data.shape})")
    ...
    # -----------------------------------------------------------------------
    def draw_x0(rank, base_seed=12346):
