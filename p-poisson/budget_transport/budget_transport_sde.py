...
def main():
    ...
    def drift_to_score(drift):
        ...
        def score(t, x):
            t_sq = t.squeeze(-1)                             # (n,)
            safe_t = jnp.clip(t_sq, 1e-6)                   # avoid /0 at t=0 for linear interpolant
            a_t = alpha_t_vmap(safe_t)[:, None]              # (n, 1)
            b_t = beta_t_vmap(safe_t)[:, None]              # (n, 1)
            ad_t = alpha_dot(safe_t)[:, None]               # (n, 1)
            bd_t = beta_dot(safe_t)[:, None]                # (n, 1)
            W = a_t * bd_t - ad_t * b_t                     # (n, 1)
            num = b_t * drift(jnp.hstack([t, x])) - bd_t * x  # (n, d); velocity uses original t
    ...
    for abs_idx, cond_val in tqdm(zip(abs_indices, ncond_vals), total=N_CHAINS, desc="sampling"):
        chunks = []
        u0_remaining = u0_cond
        for start in range(0, nsamples, chunk_size):
            this_chunk = min(chunk_size, nsamples - start)
            key2, kchunk = jax.random.split(key2)
            chunk = conditional_sample(
                velocity=velocity,
                score=score,
                cond_values=cond_val,
                yu_dimension=yu_dimension,
                reference_sampler=gaussian_reference_sampler,
                u0_cond=u0_remaining[:this_chunk],
                nsamples=this_chunk,
                solver_args=sde_solver_args,
                key=kchunk,
            )
            chunks.append(np.array(chunk))
            u0_remaining = u0_remaining[this_chunk:]
        cond_samples = np.concatenate(chunks, axis=0)

        u_pca_gen   = cond_samples[:, yu_dimension[0]:]          # (nsamples, k_pca)
        ...
        out_path = os.path.join(output_dir, f"si_sde_samps_{abs_idx:03d}.npy")
