# -*- coding: utf-8 -*-
"""
hMALA sampling for the p-Poisson inverse problem — MAIN observation (data_obs.npy).
...
Usage (10 serial trials):
    for i in $(seq 0 9); do
        python ppoisson_mcmc_main.py --output_root mcmc_main --start_idx $i
    done

Usage (10 parallel trials via MPI):
    mpirun -n 10 python ppoisson_mcmc_main.py --output_root mcmc_main
"""
...
parser = argparse.ArgumentParser(description="hMALA chain — p-Poisson main observation")
parser.add_argument("--output_root", type=str, default="mcmc_main",
...
if __name__ == "__main__":
    ...
    np.save(os.path.join(output_dir, "hmala_samples_delta.npy"), vertex_samples)
