...
output_root = "mcmc_prior_init_results"
...
yobs = np.log(np.load("true_samps/y_moderate.npy"))
...
init_points = np.log(np.load("init_points_mod.npy"))
init_key = random.key(seed=99)

start = time.perf_counter()
for i in range(no_trials):
    print(f"Running trial {i+1}/{no_trials}...")
    init_key, subkey = random.split(init_key)
    idx = int(random.randint(subkey, shape=(), minval=0, maxval=len(init_points)))
    x0 = jnp.array(init_points[idx])
    print(f"  Init x0 (from init_points_mod): {jnp.exp(x0)}")
...
true_samps = np.exp(np.load("true_samps/true_us_moderate_obs_2.npy")[::20, :])
