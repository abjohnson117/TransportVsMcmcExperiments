...
# ---------------------------------------------------------------------------
# Reference sampler (draws from log-normal prior, then normalizes to unit Gaussian)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Trig interpolant with SDE noise
# ---------------------------------------------------------------------------
def gamma_fn(t):
...
@vmap
def trig_interpolant_noise(t, x1, x0, z):
    return (
...
@vmap
def trig_interpolant_der_noise(t, x1, x0, z):
    return (jnp.pi / 2) * (
...
run = wandb.init(
    project="LV Convergence - SDE - Gauss - Trig",
    name=f"run={RANK}-sde-converge-trig",
)
...
output_root1 = "converge_results_sde_trig_1_regimes"
output_dir1 = os.path.join(output_root1, f"run_{RANK:02d}")
os.makedirs(output_dir1, exist_ok=True)

output_root2 = "converge_results_sde_trig_2_regimes"
output_dir2 = os.path.join(output_root2, f"run_{RANK:02d}")
os.makedirs(output_dir2, exist_ok=True)

output_root3 = "converge_results_sde_trig_3_regimes"
...
gamma_mmd = median_heuristic_sigma_jax(us_base, samps1)
k1 = get_gaussianRBF(gamma_mmd)
k2 = get_gaussianRBF(gamma_mmd - 6.0)
k3 = get_gaussianRBF(gamma_mmd - 3.0)
k4 = get_gaussianRBF(gamma_mmd + 3.0)
k5 = get_gaussianRBF(gamma_mmd + 6.0)
c1 = [0.2] * 5
...
for i, sample_no in tqdm(enumerate(sample_no_list)):
    ...
    v_optimizer = optax.chain(
        ...
        optax.adamw(v_schedule, weight_decay=best_hyperparams["v_weight_decay"]),
    )
    s_optimizer = optax.chain(
        optax.clip_by_global_norm(1.0),
        optax.adam(s_schedule),
    )
    ...
    wandb.log({"relative error (swd): norm": wd0_array[i]}, step=sample_no)
    wandb.log({"relative error (swd): mod":  wd2_array[i]}, step=sample_no)
    wandb.log({"relative error (swd): rare": wd3_array[i]}, step=sample_no)
    ...
    del trainer, velocity, score, v_optimizer, s_optimizer, cond_samples
    jax.clear_caches()
    gc.collect()
