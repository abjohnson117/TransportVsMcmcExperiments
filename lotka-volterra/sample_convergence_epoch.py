...
run = wandb.init(
    project="LV Convergence - ODE - Gauss - epoch",
    name=f"run={RANK}-ode-converge",
)

# Defining hyperparams
with open("hyperparam_results/best_hyperparams.pkl", "rb") as f:
    best_hyperparams = pickle.load(f)

if best_hyperparams["interpolant"] == "linear_interpolant":
    interpolant = linear_interpolant
    interpolant_der = linear_interpolant_der
elif best_hyperparams["interpolant"] == "trig_interpolant":
    interpolant = trig_interpolant
    interpolant_der = trig_interpolant_der
elif best_hyperparams["interpolant"] == "sigmoid_interpolant":
...
swd_seed = 42
n_projections = 2048
base_swd1 = 0.5581754850247667
base_swd2 = 0.5823165406083023
base_swd3 = 0.8235235490652478
